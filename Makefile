...
X11APPJAIL_VERSION?=0.1.0

all: install

install:
	${MKDIR} -m 755 -p "${DESTDIR}${PREFIX}/share/x11appjail"
	${MKDIR} -m 755 -p "${DESTDIR}${MANDIR}/man1"
	${MKDIR} -m 755 -p "${DESTDIR}${MANDIR}/man5"
	${MKDIR} -m 755 -p "${DESTDIR}${PREFIX}/bin"
	${MKDIR} -m 755 -p "${DESTDIR}${PREFIX}/share/x11appjail/appscript"
	${FIND} share/x11appjail/appscript -mindepth 1 -exec ${INSTALL} -m 555 {} "${DESTDIR}${PREFIX}/{}" \;
	${MKDIR} -m 755 -p "${DESTDIR}${PREFIX}/share/x11appjail/scaffold"
...
.endfor
	${MKDIR} -m 755 -p "${DESTDIR}${PREFIX}/lib/x11appjail"
	${INSTALL} -m 644 lib/common "${DESTDIR}${PREFIX}/lib/x11appjail/common"
	${MKDIR} -m 755 -p "${DESTDIR}${PREFIX}/libexec/x11appjail"
...
.endfor
	...
	${MKDIR} -m 755 -p "${DESTDIR}${PREFIX}/libexec/x11appjail/service.d"
.for s in OpenURL Notification
	${MKDIR} -m 755 -p "${DESTDIR}${PREFIX}/libexec/x11appjail/service.d/${s}"
	cd libexec/service.d/${s}; ${FIND} . -mindepth 1 -exec ${INSTALL} -m 555 {} "${DESTDIR}${PREFIX}/libexec/x11appjail/service.d/${s}/{}" \;
.endfor
	${MKDIR} -m 755 -p "${DESTDIR}${PREFIX}/libexec/x11appjail/misc"
...
uninstall:
	...
	${RM} -rf "${DESTDIR}${PREFIX}/share/x11appjail"
