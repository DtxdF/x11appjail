...
sed -i '' -Ee "s|^Exec=evince %U|Exec=${X11APPJAIL_APPDIR}/START %u|" "${HOME}/.local/share/applications/${DESKTOPFILE}" || exit $?
sed -i '' -Ee "s|^TryExec=evince|TryExec=${X11APPJAIL_APPDIR}/START|" "${HOME}/.local/share/applications/${DESKTOPFILE}" || exit $?
sed -i '' -Ee "s|^Exec=evince --new-window|Exec=${X11APPJAIL_APPDIR}/START --new-window|" "${HOME}/.local/share/applications/${DESKTOPFILE}" || exit $?
