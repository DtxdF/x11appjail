...
sed -i '' -Ee "s|^Exec=firefox (.+)|Exec=${X11APPJAIL_APPDIR}/START \1|" "${HOME}/.local/share/applications/${X11APPJAIL_PROFILE}-${DESKTOPFILE}" || exit $?
...
sed -i '' -Ee "s|^Icon=.+|Icon=${ICONDIR}/firefox.png|" "${HOME}/.local/share/applications/${X11APPJAIL_PROFILE}-${DESKTOPFILE}" || exit $?
appjail cmd local "${X11APPJAIL_JAIL}" cp -a usr/local/lib/firefox/browser/chrome/icons/default/default48.png "${ICONDIR}/firefox.png" || exit $?
appjail cmd jaildir chown "${X11APPJAIL_UID}:${X11APPJAIL_GID}" "${ICONDIR}/firefox.png" || exit $?
