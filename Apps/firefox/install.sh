...
sed -i '' -Ee "s|^Exec=firefox (.+)|Exec=${X11APPJAIL_APPDIR}/START \1|" "${HOME}/.local/share/applications/${DESKTOPFILE}" || exit $?
...
sed -i '' -Ee "s|^Icon=.+|Icon=${ICONDIR}/firefox.png|" "${HOME}/.local/share/applications/${DESKTOPFILE}" || exit $?
