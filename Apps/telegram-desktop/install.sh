...
sed -i '' -Ee "s|^Exec=Telegram (.+)|Exec=${X11APPJAIL_APPDIR}/START \1|" "${HOME}/.local/share/applications/${DESKTOPFILE}" || exit $?
sed -i '' -Ee "s|^TryExec=Telegram|TryExec=${X11APPJAIL_APPDIR}/START|" "${HOME}/.local/share/applications/${DESKTOPFILE}" || exit $?
