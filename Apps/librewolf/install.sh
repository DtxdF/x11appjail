...
sed -i '' -Ee "s|^Exec=librewolf (.+)|Exec=${X11APPJAIL_APPDIR}/START \1|" "${HOME}/.local/share/applications/${X11APPJAIL_PROFILE}-${DESKTOPFILE}" || exit $?
...
sed -i '' -Ee "s|^Icon=.+|Icon=${ICONDIR}/librewolf.png|" "${HOME}/.local/share/applications/${X11APPJAIL_PROFILE}-${DESKTOPFILE}" || exit $?
