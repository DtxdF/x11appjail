...
sed -i '' -Ee "s|^Exec=thunderbird (.+)|Exec=${APPDIR}/START \1|" "${HOME}/.local/share/applications/${DESKTOPFILE}" || exit $?
...
appjail cmd jaildir chown "${UID}:${GID}" "${ICONDIR}/thunderbird.png" || exit $?
